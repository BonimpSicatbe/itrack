    ...
    @if($show)
        <div class="fixed top-4 right-4 z-50 w-80 space-y-3" style="display: flex; flex-direction: column; align-items: flex-end;">
            @foreach ($messages as $index => $message)
                <div 
                    x-data="{ show: true, isExpanded: false }" 
                        ...
                        <div class="w-full">
                            <div class="flex items-start">
                                ...
                                <div class="ml-2 flex-1 min-w-0">
                                    <p :class="isExpanded ? 'whitespace-normal break-words' : 'line-clamp-2'" 
                                        class="text-sm font-medium">
                                        {{ $message['content'] }}
                                    </p>
                                    
                                    <!-- Show more/less toggle for long messages -->
                                    <template x-if="{{ strlen($message['content']) > 120 }}">
                                        <button class="text-xs mt-1 font-semibold hover:underline focus:outline-none" 
