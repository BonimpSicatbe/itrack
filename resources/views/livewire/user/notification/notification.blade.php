<div class="flex h-full bg-white rounded-lg">
    ...
    <div class="w-2/3 p-4">
        @if ($selectedNotification)
                            ...
                            <table class="table table-sm w-full">
                                ...
                                <tbody>
                                    ...
                                    @forelse ($mediaFiles as $media)
                                        <tr>
                                            <td>
                                                <a href="{{ $media->getFullUrl() }}" target="_blank" class="text-indigo-500 hover:underline">
                                                    {{ $media->name }}
                                                </a>
                                            </td>
                                            <td class="text-xs text-gray-400">
                                                {{ $media->mime_type }} ({{ \Illuminate\Support\Str::upper($media->extension) }})
                                            </td>
                                            <td class="text-xs text-gray-400">
                                                {{
                                                    (function($size) {
                                            ...
                                            <td>
                                                <a href="{{ $media->getFullUrl() }}" download class="text-blue-500 hover:underline">Download</a>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-sm text-gray-400 text-center">No required files listed.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
            ...
            @else
                <div class="text-gray-500">Requirement details not found.</div>
            @endif
        @else
            <div class="text-gray-400">Select a notification to view details.</div>
        @endif
    </div>
</div>
