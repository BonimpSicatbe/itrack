        ...
        <div class="flex-1 bg-white rounded-lg shadow-sm overflow-hidden">
            ...
            <div class="flex-1 bg-white rounded-xl overflow-y-auto p-6" style="max-height: calc(100vh - 125px);">
                ...
                @else
                            ...
                            <div class="flex flex-col sm:flex-row items-end gap-4 flex-1">
                                ...
                                <div class="flex-1 lg:max-w-96 w-full sm:w-auto">
                                    <div class="relative">
                                        ...
                                        <input id="recentSubmissionsSearch" type="text"
                                        ...
                                        <div wire:loading wire:target="search"
                                            class="absolute inset-y-3 right-0 pr-4 flex items-center">
                                            <div
                                                class="animate-spin rounded-full h-4 w-4 border-2 border-[#1C7C54] border-t-transparent">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {{-- Submissions List --}}
                    <div
                        class="{{ $viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4' : 'divide-y divide-gray-200' }}">
                        @if ($recentSubmissions->count() > 0)
                            @foreach ($recentSubmissions as $submission)
                                @if ($viewMode === 'list')
                                    <div
                                        class="mb-4 rounded-xl shadow-sm border border-gray-300 bg-white hover:border-2 hover:border-green-600">
                                        <div wire:click="showSubmissionDetail({{ $submission->id }})"
                                            ...
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center space-x-4">
                                                    <div class="flex-shrink-0">
                                                        @if ($submission->submissionFile)
                                                            <div class=" flex items-center justify-center">
                                                                <i
                                                                    class="fas {{ $submission->getFileIcon() }} {{ $submission->getFileIconColor() }} text-3xl"></i>
                                                            </div>
                                                        @else
                                                            <div
                                                                class="h-10 w-10 rounded-md bg-gray-100 flex items-center justify-center">
                                                                <i
                                                                    class="fa-solid fa-file-circle-question text-gray-400 text-lg"></i>
                                                            </div>
                                                        @endif
                                                    </div>
                                                    <div class="min-w-0 flex-1">
                                                        <p class="text-sm font-semibold text-gray-800 truncate">
                                                            ...
                                                            @else
                                                                {{ $submission->requirement->name }} <span
                                                                    class="text-gray-400 text-xs">(No file)</span>
                                                            @endif
                                                        </p>
                                                        <p class="text-xs text-gray-500 mt-1">
                                                            {{ $submission->requirement->name }} • Submitted
                                                ...
                                                <div class="flex flex-col items-end space-y-1">
                                                    <span
                                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold {{ $submission->status_badge }}">
                                    ...
                                    <div wire:click="showSubmissionDetail({{ $submission->id }})"
                                        ...
                                        <div class="flex-grow flex flex-row items-center gap-4 mb-4">
                                            <div class="flex-shrink-0">
                                                @if ($submission->submissionFile)
                                                    <i
                                                        class="fas {{ $submission->getFileIcon() }} {{ $submission->getFileIconColor() }} text-4xl"></i>
                                                @else
                                                    <i
                                                        class="fa-solid fa-file-circle-question text-gray-400 text-4xl"></i>
                                                @endif
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-semibold text-gray-800 truncate"
                                                    title="{{ $submission->submissionFile ? $submission->submissionFile->file_name : $submission->requirement->name }}">
                                                    @if ($submission->submissionFile)
                                                        {{ $submission->submissionFile->file_name }}
                                                    @else
                                                        {{ $submission->requirement->name }} <span
                        ...
                        @else
                            <div
                                class="text-center py-12 text-gray-500 {{ $viewMode === 'grid' ? 'col-span-full' : '' }}">
                                @if ($search || $statusFilter)
                                    <i class="fa-solid fa-folder-open text-4xl mb-2 text-gray-300"></i>
                                    <p class="font-semibold text-sm">No submissions found</p>
                                    <p class="text-sm font-semibold text-amber-500 mt-1">Try adjusting your search or
                                        filter</p>
                                @else
