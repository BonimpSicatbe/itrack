                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button type="button" 
                                    ...
                                    <div class="w-5 h-5 rounded border border-blue-400 flex items-center justify-center {{ $selectAllRequirements ? 'bg-blue-500 border-blue-500' : 'bg-white' }}">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                @foreach($this->requirementTypes as $type)
                                    @if($type->is_folder && $type->children->isNotEmpty())
                                        ...
                                        <!-- Folder Card -->
                                        <div class="{{ $isFolderCreated ? 'bg-white border-gray-200 opacity-100' : ($isAnyChildSelected ? '' : 'bg-white') }}">
                                            <div class="flex items-center justify-between mb-4">
                                                <div class="flex items-center gap-3">
                                                    <i class="fa-solid fa-folder {{ $isFolderCreated ? 'text-gray-500' : ($isAnyChildSelected ? 'text-yellow-500' : 'text-yellow-500') }} text-lg"></i>
                                                    <div>
                                                        <h5 class="font-medium {{ $isFolderCreated ? 'text-gray-500' : 'text-gray-700' }}">{{ $type->name }}</h5>
                                                ...
                                                <div class="flex items-center gap-2">
                                                    @if(($isMidtermFolder || $isFinalsFolder) && !$isFolderCreated && $availableChildrenCount > 0)
                                                        <button type="button"
                                                            ...
                                                            <div class="w-5 h-5 rounded border border-orange-400 flex items-center justify-center {{ ($isMidtermFolder && $selectAllMidterm) || ($isFinalsFolder && $selectAllFinals) ? 'bg-orange-500 border-orange-500' : 'bg-white' }}">
                                            ...
                                            <div class="grid grid-cols-1 gap-2">
                                                @foreach($type->children as $child)
                                                    ...
                                                    <div class="tooltip {{ $isChildCreated ? 'tooltip-warning' : '' }} flex items-center gap-3 p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer {{ $isChildCreated ? 'bg-gray-100 border-gray-200 opacity-75 cursor-not-allowed' : ($isChildChecked ? 'bg-green-50 border-green-400' : 'bg-white border-gray-200 hover:border-green-500') }}"
                                                        ...
                                                        <div class="w-5 h-5 rounded border flex items-center justify-center {{ $isChildChecked ? 'bg-green-500 border-green-500' : 'border-gray-400' }}">
                                                            @if($isChildChecked)
                                                                <i class="fa-solid fa-check text-white text-xs"></i>
                                                            @endif
                                                        </div>
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <div class="flex items-center gap-3">
                                    <!-- College Filter -->
                                    <div class="flex items-center gap-2">
                                        <label class="text-sm font-medium text-gray-700 whitespace-nowrap">Filter by College:</label>
                                        <select wire:model.live="selectedCollege" class="select select-bordered select-sm border-gray-300 rounded-lg focus:border-purple-500 focus:ring-purple-500">
                                ...
                                @if($selectedCollege)
                                    <div class="text-sm text-gray-600 mb-3">
                                        ...
                                        <span class="text-gray-500 ml-2">
                                            ({{ count($this->programs) }} program{{ count($this->programs) !== 1 ? 's' : '' }})
                                        </span>
                                    </div>
                                @endif
    ...
    <x-modal name="confirm-requirement-creation" :show="$showConfirmationModal" maxWidth="6xl">
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button wire:click="closeConfirmationModal" class="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <div class="bg-blue-50 rounded-xl p-4 mb-6 border border-blue-200">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-gray-800 font-medium">{{ \Carbon\Carbon::parse($due)->format('M j, Y g:i A') }}</p>
                    </div>
                    <div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="badge bg-blue-100 text-blue-800 border-0 font-medium px-3 py-1 rounded-full text-sm">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            @foreach($type->children as $child)
                                                @if(in_array($child->id, $selectedChildren))
                                                    <div class="flex items-center gap-2 text-sm bg-green-50 text-green-800 px-3 py-1 rounded-lg border border-green-200 ml-3">
                                                        <i class="fa-solid fa-check text-green-600 text-xs"></i>
                                                        <span class="truncate font-medium text-xs">{{ $child->name }}</span>
                                                    </div>
                                                @endif
                                            @endforeach
                                        </div>
                ...
                <div class="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="badge bg-green-100 text-green-800 border-0 font-medium px-3 py-1 rounded-full text-sm">
                            @if($selectAllPrograms)
                                All ({{ count($this->programs) }})
                            @else
                                {{ count(array_intersect($selectedPrograms, $this->programs->pluck('id')->toArray())) }} selected
                            @endif
                        </span>
                    </div>
