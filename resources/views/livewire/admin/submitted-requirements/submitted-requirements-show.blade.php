    ...
    <div class="w-full md:w-2/5 flex flex-col gap-6 overflow-y-auto pr-2 h-full">
        ...
        <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition p-0 overflow-hidden">
            ...
            <div class="p-6 space-y-4 text-sm text-gray-900">
                <div>
                    ...
                    <p class="mt-1 text-gray-500 font-semibold">{{ $submittedRequirement->requirement->name }}</p>
                </div>

                @if($submittedRequirement->requirement->description)
                <div>
                    <h3 class="font-semibold uppercase text-xs">Description</h3>
                    <p class="mt-1 text-gray-500 whitespace-pre-line font-semibold">{{ $submittedRequirement->requirement->description }}</p>
                </div>
                @endif
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-gray-500 font-semibold">{{ $submittedRequirement->requirement->due->format('M j, Y \a\t g:i A') }}</p>
                    </div>
                    <div>
                        <h3 class="font-semibold uppercase text-xs">Created By</h3>
                        <p class="mt-1 text-gray-500 font-semibold">{{ $submittedRequirement->requirement->creator->full_name }}</p>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-md hover:shadow-lg transition flex-1 flex flex-col overflow-hidden">
            ...
            <div class="text-gray-800 px-6 py-3 flex items-center justify-between rounded-t-xl border-b border-gray-300">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold text-gray-800">
                                        ...
                                        <button wire:click="selectFile('{{ $file['id'] }}')"
                                            class="w-full text-left p-3 rounded-xl transition-all duration-200 flex items-center gap-3
                                                {{ ($selectedFile && $selectedFile['id'] === $file['id']) ? 
                                                    ...
                                                    'border-1 border-gray-300 hover:border-2 hover:border-green-700' }}">
